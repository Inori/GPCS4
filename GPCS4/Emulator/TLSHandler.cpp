...
#pragma pack()
	...
	do
	{
		...
		uint8_t* pFootCode   = (uint8_t*)umemory::VMMapFlexible(nullptr, nFootCodeLen, umemory::VMPF_READ_WRITE_EXEC);
		if (!pFootCode)
		{
			break;
		}
		...
		if (!umemory::VMProtect(code, sizeof(jmpHead), umemory::VMPF_READ_WRITE_EXEC))
		{
			break;
		}
