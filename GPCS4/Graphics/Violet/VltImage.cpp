...
namespace sce::vlt
{
		...
		m_info(createInfo), m_memFlags(memFlags)
	{
		...
		if (vkCreateImage(m_device->handle(),
		...
		if (vkBindImageMemory(m_device->handle(), m_image.image,
							  m_image.memory.memory(), m_image.memory.offset()) != VK_SUCCESS)
			Logger::exception("DxvkImage::DxvkImage: Failed to bind device memory");
	}
	...
	VltImage::~VltImage()
	{
		...
		if (m_image.memory.memory() != VK_NULL_HANDLE)
			vkDestroyImage(m_device->handle(), m_image.image, nullptr);
	}

	VkSubresourceLayout VltImage::querySubresourceLayout(const VkImageSubresource& subresource) const
	{
		VkSubresourceLayout result;
		vkGetImageSubresourceLayout(
			m_device->handle(), m_image.image,
			&subresource, &result);
		return result;
	}
		...
		m_image(image), m_info(info), m_cookie(++s_cookie)
	{
		...
		default:
			Logger::exception(util::str::formatex("DxvkImageView: Invalid view type: ", m_info.type));
		}
	}
	...
	{
		for (uint32_t i = 0; i < ViewCount; i++)
			vkDestroyImageView(m_device->handle(), m_views[i], nullptr);
	}

	void VltImageView::createView(VkImageViewType type, uint32_t numLayers)
	{
		...
		if (vkCreateImageView(m_device->handle(),
