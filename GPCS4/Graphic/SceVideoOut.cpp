...
void SceVideoOut::copyPixelToVkImage(void* pixels, uint width, uint height, VkImage& dstImage)
{
	...
	createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, srcBuffer, srcBufferMemory);

	void* data;
	vkMapMemory(device, srcBufferMemory, 0, imageSize, 0, &data);
	...
	copyBufferToImage(srcBuffer, dstImage, static_cast<uint32_t>(width), static_cast<uint32_t>(height));
...
void SceVideoOut::initWindow()
{
	...
	window = glfwCreateWindow(m_nWidth, m_nHeight, "Vulkan", nullptr, nullptr);
...
void SceVideoOut::drawFrame(uint idx)
{
	...
	copyPixelToVkImage(m_vtDispBuffers[idx], m_nWidth, m_nHeight, swapChainImages[imageIndex]);
